...
import ListButtons from "../../components/outListSideButtons";
...
function Out() {
  ...
  const handleDelete = async (permitId) => {
    try {
      ...
      await API.get("/out/deleteOutPermit/", {
        params: { PermitId: permitId },
      });
  ...
  const handleInnonTransmit = async () => {
    ...
    try {
      ...
      setSelectedPermits([]);
      setInnonDecTypeSelected("");
      setShowInnonTransmitData(false);
      fetchTableData(showAll);
    } catch (error) {
