  ...
  try {
    ...
    switch (type) {
      ...
      case "storagelocation":
        response = await API.get("/getStorageLocation/");
        break;
      default:
        response = { data: [] };
    }
...
export const currentPopup = (popupType, setters) => {
  ...
  return {
    ...
    dischargeport: {
      ...
      columns: ["PortCode", "PortName", "Country"],
      onSelect: (item) => {
        setDischargePort(item);
        setDischargePortCode(item.PortCode);
        setDischargePortName(item.PortName);
      },
    },
    storagelocation: {
      title: "STORAGE LOCATION",
      columns: ["Code", "StorageCode", "Description"],
      onSelect: (item) => {
        setStorageLocation(item);
        setStorageCode(item.StorageCode);
        setStorageLocationDescription(item.Description);
      },
    },
    nextport: {
      title: "NEXT PORT",
      ...
      onSelect: (item) => {
        setNextPort(item);
        setNextPortCode(item.PortCode);
        setNextPortName(item.PortName);
      },
    },
    lastport: {
      title: "LAST PORT",
      ...
      onSelect: (item) => {
        setLastPort(item);
        setLastPortCode(item.PortCode);
        setLastPortName(item.PortName);
      },
    },
  }[popupType];
};
