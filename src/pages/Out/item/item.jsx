...
setFilteredHsCodeSuggestions,
  ...
  const deleteCascRow = async (itemIndex, rowIndex) => {
    ...
    try {
      ...
      await API.delete(
        `out/deleteOutCascByCascId/${cascId}/${rowNo}/${permitId}/`,
      );
      console.log("Deleted from OutCasc as well");
      const updated = [...itemCasc];
      updated[itemIndex].casc.splice(rowIndex, 1);
      setItemCasc(updated);
    } catch (error) {
      ...
      if (commonDeleted) {
        alert(
          `Warning: Casc row (${cascId}, row ${rowNo}) was deleted from the Common table but FAILED to delete from OutCasc. ` +
  ...
  const invoiceTotalLineAmountFunction = (
    ...
    const T1 = Number(overrideTotalLine ?? totalLineAmount) || 0;
    const T2 = Number(overrideExRate ?? invoiceExRate) || 0;
  ...
  const ItemSave = async () => {
    ...
    try {
      ...
      // const outRes = await API.post("out/postOutItemTable/", payload);
      // console.log("Saved to ItemDtl:", outRes.data);

      // if (cascData.length > 0) {
      //   await API.post("out/postOutCascTable/", cascData);
      //   console.log("CASC Data Saved Successfully (Out)");
      // }

      // two logics
      // const res = await API.post("out/postOutItemTable/", payload);
      // setItemTable(res.data.Records);
      ...
      // const cascData = ItemCascSave(itemNumber);
      // if (cascData.length > 0) {
      //   // Single call — backend saves to CommonCASCDtl AND mirrors to INNONCASCDtl
      //   const cascRes = await API.post("out/postOutCascTable/", cascData);
      //   if (cascRes.data?.Warning) {
      //     alert(cascRes.data.Warning);
      //   }
      // }

      const res = await API.post("out/postOutItemWithCasc/", payload);
      setItemTable(res.data.Records);
  ...
  //     try {
  //       await API.post("out/deleteOutItem/", {
  //         PermitId: permitId,
  //         ItemNos: selectedItems,
  //       });
  //     } catch (mirrorErr) {
  ...
  const deleteSelectedItems = async () => {
    ...
    try {
      ...
      console.log("pemritid:", res.permitId);
      setItemTable(res.data.Records);
      setSelectedItems([]);
      setSelectAll(false);
      resetItemForm();
      ...
      try {
        await API.post("out/syncOutItemFromCommon/", {
          PermitId: permitId,
        });
      } catch (mirrorErr) {
        console.error("Sync failed", mirrorErr);
        alert(
          "Warning: Items were deleted from CommonItemDtl but FAILED to sync to OutItemDtl. " +
            "Please contact support or retry.\n\n" +
            `Error: ${mirrorErr.response?.data?.error || mirrorErr.message}`,
        );
        return;
  ...
  const deleteItem = async (itemNo) => {
    ...
    try {
      ...
      try {
        ...
        alert(
          `Warning: Item No ${itemNo} was deleted from CommonItemDtl but FAILED to sync to OutItemDtl. ` +
            `Please contact support or retry.\n\nError: ${mirrorErr.response?.data?.error || mirrorErr.message}`,
        );
      }
    } catch (error) {
...
setHsQuantity(fmt(item.HSQty, 4));
    ...
    setTotalInvoiceCharge(fmt(item.InvoiceCharges, 2));
...
setTextileCategory(item.TexCat || "");
  ...
  const ItemEditAll = async () => {
    ...
    try {
      ...
      try {
        // await API.post("out/editOutAllItems/", payload);
  ...
  // if this permitid's data already exists
                ...
                <div className="row mt-2 align-items-center">
                  ...
                  <div className="col-4">
                    ...
                    >
                      ...
                      {inPackQuantityUom &&
                        !totalOuterPack?.find(
                          (t) => t.Name === inPackQuantityUom,
                        ) && (
                          <option value={inPackQuantityUom}>
                            {inPackQuantityUom}
                          </option>
                        )}
                      {totalOuterPack?.map((i, idx) => (
                ...
                <div className="row mt-2 align-items-center">
                  ...
                  <div className="col-4">
                    ...
                    >
                      ...
                      {innerPackQuantityUom &&
                        !totalOuterPack?.find(
                          (t) => t.Name === innerPackQuantityUom,
                        ) && (
                          <option value={innerPackQuantityUom}>
                            {innerPackQuantityUom}
                          </option>
                        )}
                      {totalOuterPack?.map((i, idx) => (
                ...
                <div className="row mt-2 align-items-center">
                  ...
                  <div className="col-4">
                    ...
                    >
                      ...
                      {immostPackQuantityUom &&
                        !totalOuterPack?.find(
                          (t) => t.Name === immostPackQuantityUom,
                        ) && (
                          <option value={immostPackQuantityUom}>
                            {immostPackQuantityUom}
                          </option>
                        )}
                      {totalOuterPack?.map((i, idx) => (
        ...
        <div className="col-12">
          ...
          <div className="row">
            ...
            <div className="col-4">
              ...
              <div className="row mt-3">
                <div className="col-9">
                  <DateField value={manuDate} setValue={setManuDate} />
                  {/* <input
                    type="text"
                    className="inputStyle CoTypeEmpty"
                    id="ManuDate"
              ...
              <div className="row mt-3">
                <div className="col-9">
                  <DateField value={invDate} setValue={setInvDate} />
