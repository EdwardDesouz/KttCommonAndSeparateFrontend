...
function Summary({ setActiveTab, isViewMode }) {
  ...
  const runValidation = () => {
    ...
    setShowExporterCrueiError(false);
    if (!exporterCode) {
      errors.party.push("CHECK THE IMPORTER NAME");
      setShowExporterCrueiError(true);
      isValid = false;
    } else {
      setShowExporterCrueiError(false);
    }
  ...
  const handleSavePermit = async () => {
    ...
    try {
      // -- EXPORTER (always shown in COO, required) --
      ...
      // // -- HANDLING AGENT (only if filled) --
      // if (handlingAgentCode && handlingAgentCode.trim() !== "") {
      //   const res = await API.get("/getCommonHandlingAgentTableInfo/");
      //   const exists = res.data.some(
      //     (i) =>
      //       String(i.Code).toLowerCase() ===
      //       handlingAgentCode.trim().toLowerCase(),
      //   );
      //   if (!exists)
      //     missing.push({
      //       label: "HANDLING AGENT",
      //       code: handlingAgentCode.trim(),
      //     });
      // }
  ...
  const doSavePermit = async () => {
    ...
    try {
      ...
      try {
        await API.post("coo/postCooHeaderTable/", headerPayload);
      } catch (mirrorErr) {
        console.error("Mirror header save failed", mirrorErr);
        alert(
          "Warning: Permit was saved to CommonHeaderTbl but FAILED to mirror to CoHeadertable. " +
      ...
      <div className="col-12">
        ...
        <div className="row align-items-center compact-row">
          ...
          <div className="row align-items-center compact-row">
            ...
            <div className="col-sm-2">
              {(itemResult.length > 0 ? itemResult : [{}]).map(
                (item, index) => (
        ...
        <div className="col-12">
          ...
          <div className="row mt-2">
            ...
            <div className="col-6">
              <div className="row">
                <div className="col-6 fw-semibold">CURRENCY CODE</div>
                <div className="col-6">{currencyCode}</div>
              </div>
            </div>
          </div>
