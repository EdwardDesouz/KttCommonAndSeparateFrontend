...
import { CooProvider, useCoo } from "./context/cooContext";
import CooEditLoader from "./cooEditLoader";

// ── Inner component has access to both location AND context ──────────────────
function CooNewInner({ permitId, isViewMode, isEditMode }) {
  const navigate = useNavigate();
  const location = useLocation();
  const { permitDetails } = useCoo();
...
function CooNew() {
  ...
  const location = useLocation();
  const isViewMode = location.pathname.includes("/coo/view/");
  const isEditMode = Boolean(permitId) && !isViewMode;

  return (
    <CooProvider key={permitId || "new"}>
      <CooNewInner
        permitId={permitId}
        isEditMode={isEditMode}
        isViewMode={isViewMode}
