...
import { InnonpaymentProvider } from "./context/innonpaymentContext";
import InnonpaymentEditLoader from "./innonpaymentEditLoader";
import { useInnonpayment } from "./context/innonpaymentContext";
import API from "../../api/api";
// ── Inner component has access to both location AND context ──────────────────
function InnonpaymentNewInner({ permitId, isEditMode, isViewMode }) {
  const navigate = useNavigate();
  const location = useLocation();
  const { permitDetails } = useInnonpayment();
...
function InnonpaymentNew() {
  const { permitId } = useParams();
  const location = useLocation();
  const isViewMode = location.pathname.includes("/innonpayment/view/");
  const isEditMode = Boolean(permitId) && !isViewMode;

  return (
    <InnonpaymentProvider key={permitId || "new"}>
      <InnonpaymentNewInner
        permitId={permitId}
        isEditMode={isEditMode}
        isViewMode={isViewMode}
      />
...
export default InnonpaymentNew;
