...
import { useTranshipment } from "../pages/Transhipment/context/transhipmentContext";
...
function ListButtons({
  ...
  const { permitDetails, updatePermitDetails } = useTranshipment();
  ...
  const handleClick = async (label) => {
    try {
      if (label === "NEW") {
        ...
        const res = await API.get(`transNew/?user=${currentUsername}`);
      ...
      if (label === "COPY") {
        ...
        const res = await API.post("copyTranshipment/", {
        ...
        if (res.data?.SUCCESS) {
          ...
          if (copiedPermits.length === 1) {
            try {
              ...
              navigate(`/transhipment/edit/${copiedPermits[0]}`, {
      ...
      if (label === "AMEND") {
        ...
        try {
          ...
          const res = await API.post("copyTransAmend/", {
            permits: [permitId],
            user: currentUsername,
          });
      ...
      if (label === "CANCEL") {
        ...
        try {
          ...
          const res = await API.post("copyTransCancel/", {
            permits: [permitId],
            user: currentUsername,
          });
      ...
      if (label === "REFUND") {
        ...
        try {
          ...
          // if (setSelectedPermits) setSelectedPermits([]);
          if (clearSelection) clearSelection();
          navigate(`/transhipment/edit/${permitId}`, {
            state: {
