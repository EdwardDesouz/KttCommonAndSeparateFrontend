...
import { useCoo } from "../pages/Coo/context/cooContext";
...
function ListButtons({
  ...
  const { permitDetails, updatePermitDetails } = useCoo();
  ...
  const handleClick = async (label) => {
    try {
      if (label === "NEW") {
        ...
        const res = await API.get(`cooNew/?user=${currentUsername}`);
      ...
      if (label === "COPY") {
        ...
        const res = await API.post("copyCoo/", {
        ...
        if (res.data?.SUCCESS) {
          ...
          if (copiedPermits.length === 1) {
            try {
              ...
              navigate(`/coo/edit/${copiedPermits[0]}`, {
